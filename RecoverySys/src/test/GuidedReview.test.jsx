...
const state = {
  ...
  config: {
    main_chute: { id: 'cl-24-n', category: 'main_chute', name: 'Catalog main', manufacturer: 'b2 Rocketry Company' },
    drogue_chute: { id: 'custom-drogue', category: 'drogue_chute', name: 'User drogue', manufacturer: 'Custom' },
  },
  simulation: null,
}
...
describe('GuidedReview first-plan entry', () => {
  it('keeps plan actions and value sources explicit', () => {
    ...
    expect(screen.getByRole('heading', { name: /set the scope of your first plan/i })).toBeInTheDocument()
  ...
  it('keeps result status conservative and source-input navigation available', () => {
    ...
    render(<GuidedReview state={state} onOpenSimulation={onOpenSimulation} onOpenSpecs={onOpenSpecs} />)
