...
describe('canonical assessment contract', () => {
  ...
  it('distinguishes current and stale evaluated outputs', () => {
    expect(buildAssessment({ domain: 'recovery', output: 'main_fps', value: 12, unit: 'ft/s', fresh: true })).toMatchObject({
...
describe('stable finding contract', () => {
  it('does not derive compatibility identity from dynamic values', () => {
    const first = compatibilityFindingCode('main_chute', 'Main descent rate 15.1 fps is above 15 fps')
    const second = compatibilityFindingCode('main_chute', 'Main descent rate 17.4 fps is above 15 fps')
