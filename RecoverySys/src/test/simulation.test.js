  ...
  it('uses layered wind integration rather than scalar surface wind', () => {
    const layered = computeDrift({
      ...
      specs: {
        ...
        wind_mid_speed_mph: '40',
        wind_mid_direction_deg: '180',
        wind_mid_alt_ft: '2500',
      },
    })
    const surfaceOnly = computeDrift({
      simulation: baseSim,
      specs: { wind_speed_mph: '10', wind_direction_deg: '0' },
    })
...
describe('runSimulation', () => {
  ...
  it('rejects malformed prefixes at the simulation boundary', () => {
    expect(
      runSimulation({ specs: { ...baseSpecs, rocket_mass_g: '2500g' }, config: baseConfig })
    ).toBeNull()
  })

  it('uses layered wind for scalar drift when a profile is supplied', () => {
    const scalar = runSimulation({
      specs: { ...baseSpecs, wind_direction_deg: '0' },
      config: baseConfig,
    })
  ...
  it('uses the shared exact drift route, and rejects missing direction everywhere', () => {
    ...
    expect(
      computeDrift({ simulation: noDirection, specs: { ...baseSpecs, wind_direction_deg: '' } })
    ).toBeNull()
  })
