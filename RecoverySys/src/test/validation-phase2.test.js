...
const finite = (value, id) =>
  expect(Number.isFinite(value), `${id}: output must be finite`).toBe(true)
...
describe('Phase 2 analytic validation foundation', () => {
  ...
  it('matches independent ISA density references', () => {
    for (const testCase of PHASE2_CASES.filter(({ id }) => id.startsWith('isa-density'))) {
      ...
      expect(
        Math.abs(actual - testCase.expected.value),
        `${testCase.id}: ISA density (${testCase.expected.unit})`
  ...
  it('matches the independent terminal-descent drag equation', () => {
    ...
    expect(
      Math.abs(actual - testCase.expected.value),
      `${testCase.id}: descent rate (${testCase.expected.unit})`
    ).toBeLessThanOrEqual(testCase.expected.tolerance)
  })
  ...
  it('matches deterministic layered-wind interpolation and meteorological drift convention', () => {
    ...
    expect(
      Math.abs(actual.drift_ft - testCase.expected.value),
      `${testCase.id}: drift (${testCase.expected.unit})`
    ).toBeLessThanOrEqual(testCase.expected.tolerance)
