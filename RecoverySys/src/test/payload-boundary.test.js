...
describe('payload boundary', () => {
  it('rejects malformed payloads', () =>
    expect(decodeMigrateValidateNormalize('{bad', options).ok).toBe(false))
  it('rejects future versions', () =>
    expect(
      decodeMigrateValidateNormalize({ ...valid, schemaVersion: SCHEMA_VERSION + 1 }, options).error
        .code
    ).toBe('future-version'))
  it('rejects oversized payloads', () =>
    expect(
      decodeMigrateValidateNormalize(
        { ...valid, specs: { rocket_mass_g: 'x'.repeat(PAYLOAD_LIMITS.jsonBytes) } },
        options
      ).error.code
    ).toBe('oversized'))
  it('rejects invalid categories instead of dropping them', () =>
    expect(
      decodeMigrateValidateNormalize({ ...valid, config: { unknown: { id: 'part' } } }, options)
        .error.code
    ).toBe('invalid-category-or-config'))
  it('preserves id and category catalog lookup', () => {
    const part = { id: 'same-id', category: 'main_chute', name: 'Catalog', specs: {} }
    const result = decodeMigrateValidateNormalize(
      { ...valid, config: { main_chute: { id: part.id } } },
      { ...options, allParts: [part] }
    )
  ...
  it('normalizes saved catalog selections using the full catalog', () => {
    localStorage.setItem(
      STORAGE_KEYS.CONFIG,
      JSON.stringify({ ...valid, config: { main_chute: { id: 'same-id' } } })
    )
    const part = { id: 'same-id', category: 'main_chute', name: 'Catalog', specs: {} }
    expect(loadSaved()?.config.main_chute).toBe(
      PARTS.find(
        (candidate) => candidate.id === 'same-id' && candidate.category === 'main_chute'
      ) ?? null
    )
    expect(
      decodeMigrateValidateNormalize(
        JSON.stringify({ ...valid, config: { main_chute: { id: part.id } } }),
        { ...options, allParts: [part] }
      ).config.main_chute
    ).toBe(part)
  })
  ...
  it('exports catalog references and imports them back through the boundary', () => {
    ...
    const exported = JSON.parse(
      encodeJsonPayload({ config: { main_chute: part }, specs: {}, customMotor: null })
    )
    expect(exported.config.main_chute).toEqual({ id: 'same-id' })
    expect(
      decodeMigrateValidateNormalize(exported, { ...options, allParts: [part] }).config.main_chute
    ).toBe(part)
  })
  ...
  it('handles share catalog references and valid inline custom parts', () => {
    ...
    const result = decodeMigrateValidateNormalize(
      { ...valid, config: { main_chute: custom } },
      options
    )
  ...
  it('uses the shared byte limit for raw JSON and file-size guards', () => {
    ...
    expect(
      decodeMigrateValidateNormalize(' '.repeat(PAYLOAD_LIMITS.jsonBytes + 1), options).ok
    ).toBe(false)
  })
  ...
  it('rejects oversized and over-limit custom-part storage', () => {
    expect(
      saveCustomPartsToStorage(
        Array.from({ length: PAYLOAD_LIMITS.customParts + 1 }, (_, i) => ({ id: `custom-${i}` }))
      )
    ).toBe(false)
    expect(
      saveCustomPartsToStorage([
        {
          id: 'custom-huge',
          name: 'x'.repeat(PAYLOAD_LIMITS.customPartsJsonBytes),
          category: 'main_chute',
          specs: {},
        },
      ])
    ...
    localStorage.setItem(
      ...
      JSON.stringify(
        Array.from({ length: PAYLOAD_LIMITS.customParts + 1 }, () => ({
          id: 'custom-x',
          name: 'x',
          category: 'main_chute',
          specs: {},
