...
export default function DispersionMap({ simulation, specs, forceOpen = false }) {
  ...
  const drift = useMemo(() => computeDrift({ simulation, specs }), [simulation, specs])
  ...
  useEffect(() => {
    ...
    getLeaflet().then((Lf) => {
      ...
      add(Lf.marker([launch_lat, launch_lon], { icon: launchIcon }).bindPopup('<b>Launch Site</b>'))
      ...
      if (land_lat != null && land_lon != null) {
        ...
        if (drogue_vector && main_vector) {
          ...
          add(
            Lf.polyline(
              ...
              `<b>Drogue Phase</b><br>${drogue_drift_ft.toLocaleString()} ft / ${drogue_time_s}s`
            )
          )
          ...
          add(
            Lf.polyline(
              ...
              `<b>Main Phase</b><br>${main_drift_ft.toLocaleString()} ft / ${main_time_s}s`
            )
          )
        }
        ...
        add(
          Lf.marker([land_lat, land_lon], { icon: landIcon }).bindPopup(
            `<b>Predicted Landing</b><br>${drift_ft.toLocaleString()} ft downwind`
          )
        )
    ...
    >
      ...
      {open && (
        <div style={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0 }}>
          ...
          {!hasCoords && drift && (
            <div
              style={{ padding: '8px 14px 4px', fontSize: '11px', color: 'var(--text-tertiary)' }}
            >
          ...
          {canShow && hasCoords && monteCarlo && (
            ...
            >
              <span>
                <span style={{ color: '#f59e0b' }}>■</span> Drogue vector
              </span>
              <span>
                <span style={{ color: '#ef4444' }}>■</span> Main vector
              </span>
              <span>
                <span style={{ color: '#ef4444', opacity: 0.5 }}>·</span> MC scatter (
                {monteCarlo.scatter.length} pts)
              </span>
              <span>
                <span style={{ color: '#f59e0b' }}>⬭</span> 95% ellipse
              </span>
            </div>
          )}
        </div>
