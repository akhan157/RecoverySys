...
export default function FlightChart({ simulation }) {
  ...
  const { timeline, apogee_ft, deploy_ft, phase1_time_s, total_time_s, apogee_t_s, burnout_t_s } =
    simulation
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: 'block', background: '#0e0e0e' }}>
      <ChartDefs />
      ...
      {(() => {
        ...
        const positions = events.map((ev) => ({ ...ev, x: xS(ev.t), row: 0 }))
        // Assign rows: if too close to a previous label on the same row, bump down
        for (let i = 1; i < positions.length; i++) {
          for (let row = 0; row < 4; row++) {
            const conflict = positions
              .slice(0, i)
              .some((prev) => prev.row === row && Math.abs(positions[i].x - prev.x) < MIN_X_GAP)
            if (!conflict) {
              positions[i].row = row
              break
            }
      ...
      <path ref={pathRef} d={d} stroke="#ffffff" strokeWidth="2" fill="none" />
    </svg>
  )
}
