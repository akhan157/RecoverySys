      ...
      <div className="mc-sim__top">
        ...
        <div className="mc-sim__chart">
          <h2 className="mc-panel-header">
            ...
            <span className="mc-panel-header__right">
              {sim && !resultFresh ? 'RESULT_STALE // RERUN_REQUIRED' : sim ? `REF_ID: STR-SIM-${String(Math.abs((sim.apogee_ft || 0) * 7 + (sim.drift_ft || 0)) % 9999).padStart(4, '0')}` : 'AWAITING_DATA'}
            </span>
          </h2>
        ...
        <div className="mc-sim__data">
          ...
          <div className="mc-sim__data-grid">
            <MetricCard label="APOGEE_ALTITUDE" value={usableSim ? usableSim.apogee_ft.toLocaleString() : '—'} unit="ft" />
            <MetricCard
              label="MAIN_DESCENT"
            ...
            />
            <MetricCard label="DESCENT_TIME" value={usableSim?.total_time_s != null ? Math.round(usableSim.total_time_s) : '—'} unit="sec" />
            <MetricCard label="DRIFT_DISTANCE" value={usableSim ? usableSim.drift_ft.toLocaleString() : '—'} unit="ft" />
