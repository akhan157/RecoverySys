...
export default function GuidedReview({
  ...
  const selectedParts = useMemo(
    () => Object.values(config).filter(Boolean),
    [config]
  )
  const hasPlan = selectedParts.length > 0 || Object.values(specs).some(hasValue)
  ...
  return (
    <section className="guided-review" aria-label="Guided first plan">
      <div className="guided-review__rail">
        ...
        <h2>Start a recovery plan<br /><em>with clear limits.</em></h2>
        <p className="guided-review__intro">
          A short path from inputs to a reviewable estimate. You can pause, resume, import,
          or use the direct navigation at any time.
        </p>
        ...
        <div className="guided-review__rail-note">
          PLAN STATE<br /><strong>{hasPlan ? 'INPUTS IN PROGRESS' : 'NEW PLAN'}</strong>
        </div>
      </div>
      ...
      <div className="guided-review__content">
        ...
        {step === 0 && (
          <div className="guided-review__page fade-up">
            ...
            <p className="guided-review__lede">
              Required values remain user supplied. Optional values may stay blank when a
            ...
            <div className="guided-review__plan-actions" aria-label="Plan actions">
              ...
              <button className="guided-review__action-card" onClick={onOpenDashboard} disabled={!hasPlan}>
                <strong>RESUME THIS PLAN</strong>
                <span>{hasPlan ? 'Keep entered values and continue in the dashboard.' : 'No entered values to resume yet.'}</span>
              </button>
            ...
            <div className="guided-review__value-grid">
              <section>
                <div className="guided-review__kicker">REQUIRED / USER SUPPLIED</div>
                <dl>
              ...
              <section>
                ...
                <dl>
                  {optionalInputs.map(input => (
                    <div key={input.label}><dt>{input.label}</dt><dd>{input.value}</dd></div>
                  ))}
                </dl>
              </section>
            </div>
            ...
            <div className="guided-review__hardware-summary">
              ...
              {selectedParts.length > 0 ? selectedParts.map(part => (
                <div className="guided-review__hardware-row" key={`${part.category}-${part.id}`}>
                  <span>{part.name || part.id}</span><small>{partSource(part)}</small>
                </div>
              )) : <p>No recovery hardware selected yet. Catalog values remain unverified; custom values remain user supplied.</p>}
            </div>

            <div className="guided-review__callout guided-review__callout--info">
              <span>i</span>
              <p><strong>This is a planning estimate, not a safety approval or certification.</strong> Review engineering, manufacturer, and range requirements separately.</p>
            </div>
          </div>
        )}
        ...
        {step === 1 && (
          <div className="guided-review__page fade-up">
            ...
            <p className="guided-review__lede">
              Results remain estimates and screening signals. Open method details only when a
              result needs context.
            </p>
            <div className="guided-review__result-list">
              <article className="guided-result">
                <div className={`guided-result__status guided-result__status--${status.tone}`}>{status.label}</div>
                <div><div className="guided-review__kicker">FLIGHT OUTCOME</div><h4>Recovery sequence estimate</h4><p>{state.simulation ? 'A simulation result is available for this plan.' : 'Run the simulation after required inputs and recovery hardware are supplied.'}</p></div>
                <button className="guided-result__action" onClick={onOpenSimulation}>OPEN</button>
              </article>
              <article className="guided-result">
                <div className="guided-result__status guided-result__status--quiet">SCREENING</div>
                <div><div className="guided-review__kicker">LANDING WINDOW</div><h4>Where might the vehicle land?</h4><p>{state.simulation?.drift_ft != null ? `Estimated drift: ${state.simulation.drift_ft.toFixed(0)} ft.` : 'Landing drift is unavailable until a current simulation exists.'}</p></div>
                <button className="guided-result__action" onClick={onOpenSimulation}>OPEN</button>
              </article>
              <article className="guided-result">
                <div className="guided-result__status guided-result__status--quiet">SCREENING</div>
                <div><div className="guided-review__kicker">HARDWARE LOADS</div><h4>What deserves a closer look?</h4><p>The available evidence supports a preliminary check, not a final structural sign-off.</p></div>
                <button className="guided-result__action" onClick={() => setShowMethod(true)}>WHY?</button>
              </article>
            </div>
            {showMethod && <div className="guided-review__callout guided-review__callout--warn"><span>!</span><p><strong>Context needed.</strong> Load signals are comparative indicators. Confirm material ratings, attachment geometry, and field procedures separately.</p></div>}
          </div>
        )}
        ...
        {step === 2 && (
          <div className="guided-review__page fade-up">
            ...
            <p className="guided-review__lede">Open this layer when you need to explain a result to a teammate or challenge an input.</p>
        ...
        <div className="guided-review__nav">
          ...
          <button className="guided-review__pause" onClick={onOpenDashboard}>PAUSE AND RETURN</button>
