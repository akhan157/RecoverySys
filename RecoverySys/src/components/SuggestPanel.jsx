...
function SuggestRow({ part, detail, isSelected, onSelect }) {
  return (
    <div
      ...
      onMouseEnter={(e) => {
        if (!isSelected) e.currentTarget.style.background = 'var(--bg-right)'
      }}
      onMouseLeave={(e) => {
    ...
    >
      <div>
        <div style={{ fontSize: '12px', fontWeight: 600, color: 'var(--text-primary)' }}>
          {isSelected && (
            <span style={{ color: 'var(--ok-fg, #4ade80)', marginRight: '4px' }}>✓</span>
          )}
          {part.name}
        </div>
...
export default function SuggestPanel({ parts, specs, config, onSelectPart }) {
  ...
  const suggestions = useMemo(() => {
    ...
    const drogueChutes = scored(parts, 'drogue_chute', (p) => {
      ...
      return {
        part,
        detail: `${fpsLabel(fps)} drogue descent · ${part.specs.diameter_in}" Ø Cd ${part.specs.cd}`,
      }
    })
    ...
    const shockCords = scored(parts, 'shock_cord', (p) => {
      ...
      return {
        part,
        detail: `${part.specs.strength_lbs} lbs · ${part.specs.length_ft}ft · ${part.specs.material} · SF ${sf}×`,
      }
    })
          ...
          {!mass_kg ? (
            ...
            <div style={{ maxHeight: '240px', overflowY: 'auto' }}>
              ...
              {suggestions.shockCords.length > 0 && (
                <SuggestSection
                  label={`Shock Cord — ≥${Math.ceil(mass_kg * G_ACCEL * g_factor * LBS_PER_N)} lbs min at ${g_factor}G`}
                >
