...
const HARNESS_MIN_LENGTH_FT = (mass_kg) => (mass_kg >= 10 ? 15 : mass_kg >= 2.5 ? 10 : 5)
...
function buildContext({ config, specs }) {
  ...
  const obstruction_vol = Math.max(
    0,
    parseSpec('bay_obstruction_vol_in3', specs.bay_obstruction_vol_in3) ?? 0
  )
  const usable_volume = bay_volume > 0 ? Math.max(0, bay_volume - obstruction_vol) : 0
  const mass_kg = mass_g > 0 ? mass_g / 1000 : null
  const deploy_alt_ft =
    parseSpec('main_deploy_alt_ft', specs.main_deploy_alt_ft) ?? DEFAULT_DEPLOY_ALT_FT
...
const err = (slot, message, metadata = {}) => ({
  level: WARN_LEVELS.ERROR,
  slot,
  message,
  ...metadata,
})
const warn = (slot, message, metadata = {}) => ({
...
function rule_openingShock(ctx) {
  ...
  const F_open_N =
    0.5 * rho_deploy * drogue_mps * drogue_mps * config.main_chute.specs.cd * main_area * Cx
...
function rule_shockCordStrength(ctx) {
  ...
  const staticCriterion = evaluateCriterion(
    CRITERION_IDS.STRENGTH_MARGIN,
    strength_lbs / required_lbs
  )
  ...
  if (length_ft != null) {
    ...
    if (length_ft < min_length) {
      out.push(
        warn(
          'shock_cord',
          `Harness length ${length_ft}ft may be insufficient — minimum ${min_length}ft recommended for this rocket class`
        )
      )
    }
  }
...
function rule_chuteCordMaterialMismatch(ctx) {
  const { config } = ctx
  if (
    config.shock_cord?.specs.material === 'kevlar' &&
    config.main_chute?.specs.material === 'nylon'
  ) {
    return [
      warn(
        'shock_cord',
        'Kevlar cord with nylon chute — mismatched elasticity. Kevlar transfers full snatch load to nylon canopy attachment points. Consider tubular nylon cord or adding a Kevlar shock cord protector/bungee section.'
      ),
    ]
  }
  return []
...
function rule_chuteProtector(ctx) {
  ...
  if (max_chute_diam_in != null && diameter_in > max_chute_diam_in) {
    return [
      err(
        'chute_protector',
        `${config.chute_protector.name} (max ${max_chute_diam_in}" chute) is too small for ${diameter_in}" main — chute may be scorched`
      ),
    ]
  }
  return []
...
function rule_quickLinksVsCord(ctx, warningsSoFar) {
  ...
  if (warningsSoFar.some((w) => w.slot === 'quick_links' && w.level === WARN_LEVELS.ERROR))
    return []
  const ql_lbs = config.quick_links.specs.strength_lbs
  ...
  if (ql_lbs < sc_lbs) {
    return [
      err(
        'quick_links',
        `Quick links rated ${ql_lbs} lbs are weaker than shock cord (${sc_lbs} lbs) — links will fail first`
      ),
    ]
  }
  if (ql_lbs < sc_lbs * SF_MARGINAL_RATIO_QL) {
    return [
      warn(
        'quick_links',
        `Quick links (${ql_lbs} lbs) are only marginally stronger than shock cord (${sc_lbs} lbs)`
      ),
    ]
  }
  return []
...
function rule_quickLinkOpeningSize(ctx) {
  ...
  if (ql_opening < cord_width) {
    return [
      err(
        'quick_links',
        `Quick link opening (~${ql_opening.toFixed(2)}") is too small for ${cord_width}" cord — cord won't thread through`
      ),
    ]
  }
  return []
...
function rule_chuteDeviceAltitudeBounds(ctx) {
  ...
  if (deploy_alt_raw > deploy_alt_max_ft) {
    return [
      err(
        'chute_device',
        `${config.chute_device.name} max altitude is ${deploy_alt_max_ft.toLocaleString()} ft — deploy altitude ${deploy_alt_raw.toLocaleString()} ft exceeds it`
      ),
    ]
  }
  if (deploy_alt_raw < deploy_alt_min_ft) {
    return [
      warn(
        'chute_device',
        `${config.chute_device.name} minimum altitude is ${deploy_alt_min_ft} ft — deploy altitude ${deploy_alt_raw} ft is below it`
      ),
    ]
  }
  return []
...
function rule_deploymentBagFit(ctx) {
  ...
  if (bag_max != null && chute_d > bag_max) {
    out.push(
      warn(
        'deployment_bag',
        `${config.deployment_bag.name} (max ${bag_max}" chute) may be too small for ${chute_d}" main — chute may not pack cleanly`
      )
    )
  }
  ...
  if (bag_depth > 0 && chute_len > 0 && chute_len > bag_depth * 1.5) {
    out.push(
      warn(
        'deployment_bag',
        `Main chute packed length ${chute_len}" may not fit in ${config.deployment_bag.name} (est. ${bag_depth.toFixed(1)}" internal depth) — chute may stick out, defeating controlled deployment`
      )
    )
  }
  return out
  ...
  for (const [slot, item] of [
    ...
    if (pd && pd > airframe_id) {
      out.push(
        err(
          slot,
          `${item.name} packed diameter ${pd}" exceeds airframe ID ${airframe_id}" — won't fit in tube`
        )
      )
    }
  }
...
function rule_singleDeploy(ctx) {
  ...
  if (config.main_chute && !config.drogue_chute) {
    return [
      warn(
        'drogue_chute',
        'No drogue chute — single deploy. Rocket will free-fall to main deploy altitude.'
      ),
    ]
  }
  return []
...
function rule_dualDeployNoBag(ctx) {
  ...
  if (config.drogue_chute && config.main_chute && !config.deployment_bag) {
    return [
      warn(
        'deployment_bag',
        'Dual-deploy without a deployment bag — main chute opens uncontrolled at drogue speed. A d-bag reduces opening shock by 30-40% via controlled line deployment.'
      ),
    ]
  }
  return []
...
function rule_noMainChute(ctx) {
  ...
  const hasAnyComponent = !!(
    ...
    config.drogue_chute ||
    config.shock_cord ||
    config.chute_protector ||
    config.deployment_bag ||
    config.quick_links ||
    config.swivel ||
    config.chute_device
  )
