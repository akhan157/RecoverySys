...
function findingState(item = {}) {
  ...
  if (raw === 'ok' || raw === 'pass' || raw === 'passed' || raw === 'clear')
...
function normalizeFinding(item, index) {
  ...
  return {
    ...
    evaluated: firstDefined(value.evaluated, state !== FINDING_STATES.NOT_EVALUATED),
...
export function summarizeReviewFindings(input = {}) {
  ...
  const counts = {
    ...
    notEvaluated: items.filter((item) => item.state === FINDING_STATES.NOT_EVALUATED).length,
  }
