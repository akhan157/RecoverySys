...
export function isValidCustomPart(part, slotIds = []) {
  return (
    ...
    part.name.length > 0 &&
    part.name.length <= PAYLOAD_LIMITS.customNameChars &&
    typeof part.category === 'string' &&
    slotIds.includes(part.category) &&
    part.specs &&
    typeof part.specs === 'object' &&
    !Array.isArray(part.specs)
  )
}
...
export function validateCustomMotor(motor) {
  ...
  if (
    typeof motor.designation !== 'string' ||
    motor.designation.length === 0 ||
    motor.designation.length > 200
  )
    return null
  if (
    !Array.isArray(motor.curve) ||
    motor.curve.length < 2 ||
    motor.curve.length > PAYLOAD_LIMITS.motorCurvePoints
  )
    return null
  ...
  for (const point of motor.curve) {
    if (
      ...
      !Number.isFinite(point.thrust_N) ||
      point.t <= previous ||
      point.t < 0 ||
      point.thrust_N < 0
    )
...
function normalizeConfig(rawConfig, { allParts = [], slotIds = [], emptyConfig = {} }) {
  if (rawConfig == null)
    return { ...emptyConfig, catalogMissing: 0, customMissing: 0, inlinedCustomParts: [] }
  ...
  for (const [category, part] of Object.entries(rawConfig)) {
    ...
    if (part.id.startsWith('custom-')) {
      if (
        isValidCustomPart(part, slotIds.length > 0 ? slotIds : [category]) &&
      ...
      const found = allParts.find(
        (candidate) => candidate.id === part.id && candidate.category === category
      )
...
export function decodeMigrateValidateNormalize(input, options = {}) {
  ...
  if (!Number.isInteger(migrated.schemaVersion) || migrated.schemaVersion !== SCHEMA_VERSION)
    return failure('unsupported-version', 'Payload schema cannot be migrated')
  const normalizedConfig = normalizeConfig(migrated.config, options)
  if (!normalizedConfig)
    return failure('invalid-category-or-config', 'Payload contains an invalid category or part')
  ...
  const customMotor =
    migrated.customMotor == null ? null : validateCustomMotor(migrated.customMotor)
  if (migrated.customMotor != null && !customMotor)
    return failure('invalid-motor', 'Payload contains an invalid custom motor')
  return {
    ok: true,
...
export function decodeShareEncoded(encoded, options = {}) {
  ...
  try {
    return decodeMigrateValidateNormalize(
      JSON.parse(decodeURIComponent(atob(decodeURIComponent(encoded)))),
      options
    )
  } catch {
    return failure('malformed', 'Share payload is malformed')
  }
}
