...
export function encodeSharePayload({ config, specs, customMotor }) {
  const configIds = Object.fromEntries(Object.entries(config).map(([cat, part]) => [cat, part ? (part.id?.startsWith('custom-') ? part : { id: part.id }) : null]))
  return btoa(encodeURIComponent(JSON.stringify({ schemaVersion: SCHEMA_VERSION, config: configIds, specs, customMotor })))
}

export function buildShareUrl(encoded) { return `${location.origin}${location.pathname}?${SHARE_PARAM}=${encodeURIComponent(encoded)}` }
