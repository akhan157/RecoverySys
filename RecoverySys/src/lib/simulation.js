 ...
 */
function cdAtMach(mach, cd_sub) {
  // constraint #2: generic 4:1 ogive curve
...
function integrateAscent(
  ...
  cd_sub,
  curve = null,
  propMass_kg_override = null
) {
  const prop_mass_kg =
    propMass_kg_override != null && propMass_kg_override > 0
      ? Math.min(propMass_kg_override, total_mass_kg * 0.6) // constraint #3: cap prevents negative dry mass
...
function interpolateWind(alt_ft, layers) {
  ...
  if (layers.length === 1)
    return { speed_mph: layers[0].speed_mph, direction_deg: layers[0].direction_deg }

  const sorted = [...layers].sort((a, b) => a.alt_ft - b.alt_ft)
  if (alt_ft <= sorted[0].alt_ft)
    return { speed_mph: sorted[0].speed_mph, direction_deg: sorted[0].direction_deg }
...
export function computeDrift({ simulation, specs }) {
  ...
  const bearing_deg = drift_ft > 0 ? ((Math.atan2(dx_ft, dy_ft) * 180) / Math.PI + 360) % 360 : null
...
export function runDispersionMonteCarlo({ simulation, specs, iterations = 500 }) {
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    const cd_factor = Math.max(Number.MIN_VALUE, 1 + 0.1 * gaussRand()) // ±10% Cd uncertainty
    ...
    if (
      !Number.isFinite(apogee_pert) ||
      !Number.isFinite(drogue_pert) ||
      !Number.isFinite(main_pert) ||
      drogue_pert <= 0 ||
      main_pert <= 0
    )
      continue
    ...
    const perturbedLayers = baseLayers.map((layer) => ({
      alt_ft: layer.alt_ft,
      speed_mph: Math.max(0, layer.speed_mph * (1 + 0.3 * gaussRand())),
...
export function runSimulation({ specs, config, customMotor = null }) {
  ...
  const timeline =
    ascentTimeline && ascentTimeline.length > 1
